# Apple Orchard
- Soil survey in images
- odd gem mount on the floor, but no gem
- can divert smudge pot power to the odd blue flame thing - I think this may stop my extra food though
- Heart on tree by orchard - CHOPPED DOWN????
- I feel like the sundial means something???

# Attic
- Note about the decline of The Red Prince - April 12
- Note about the middle left security screen? Have screenshot

# Antechamber
- Has key to basement
- Controls
  - North - Underground
  - West - Secret Garden
  - East - Great Hall
  - South - Greenhouse
- Levers
  - Greenhouse lever is broken - Broken Lever in workshop
  - West and east one is working
    - east one is in great hall

# Aquarium
- X marks the spot under the bbq in patio
- Arrow pointing at filing cabinet in archive

# Archive
- Insurgent group RLCF
- Marion Mary Marigold went missing at 3:45pm Friday
- Mary Epsen birth certificate
  - 4:05am Tuesday 22/01/1958

# Ballroom
- Music sheets
- Photo of Clara & Simon - 1952

# Basement
- Need to hit wall with power hammer
- Hundred Bells - Sandy Heights 5pm

# Billiard Room
- Blue - +
- Yellow - -
- Pink - *
- Purple - /


# Blackbridge Grotto
- Logins
  - MoorR (Admin) Randolph Moore
  - ThomK (Admin) - K Thom?
  - BabbA (Personnel Access) - Anne Babbage
  - RuotH - Expired
  - RevaD - Experied (Denny revane)
- Admins Log
  - Recording capabiltiies of all cameras has been disabled? - 02-10-87
  - Lots of deleted logs
  - New Admin key created that doesn't match the typical pattern???
- Need to find an Archive Acess to view deleted logs
- Microchip Object?
- 1 100 (binary?) - 1 4

# Boiler Room
- Has a pattern on a 4x4 grid

# Book Shop
- A New Clue by Marion Marigold
- HOLY SHIT BOOKS ARE EXPENSIVE

# Boudoir
- safe code - 2412
- Red Note - 4
  - Hebert, hid details of marions dissapearance, from Daniel

# Bunker
- Red note VII
  - Smuggled cousin (Marigold?) across the red border to the southern seafront
  - 7 months before she went missing
  - coincides with ruby crown thefta
  - From courtney
- Emails
  - 22/02/1987 - Randolph - Working to get more supplies
  - 25/02/1987 - Randolph - Idea for long term solution that may involve a boat ride
  - 28/02/1987 - Randolph - RedGuard coming tomorrow to investigate dissapearance - Too risky to move

# Catacombs
- Not much down there - Part of the underground map

# Chapel
- Light both the candels to show piggy bank

# Classroom
- Grade 1
  - CALEB (Favourite colour red)
  - Flags (2 of each colour? idk what this is hinting at)
    - Mountain - Yellow
    - Heart - Orange
    - Timer - Purple
- Grade 2
  - Old Text missing page 36, 37 (Politcal whims CBE)
  - 7, 13, 4, 1, 4, 12, 10, 4, 2, 21, 18, 15 (hint at billiard)
- Grade 3
  - Water, Ice, Steam (hint at laboratory)
- Grade 4
  - Maps?? Have screenshots
- Grade 5
  - Art class - Draw two things where one is missing a letter

# Cloister
- Switches at bottom of pillar facing statue
- C X E T (bottom of statue)

# Coat Check
- Maybe an entrance to the basement? - floor access hole

# Conference Room
- After meeting all notes put in office safe
- Safe Code - 9083? Probably wrong

# Darkroom
- with lights on and magnifying glass check out some of the images
- Has image of Tree with date 11-28
- Green Memos in Hartleys room are always false

# Den
- 2 back clocks have diagrams indicating
  - 9:15 and 11:05
  - 3:45 and 1:55

# Dining Room
- Chairs backrest pattern matches the CPU pattern in blackbridge grotto

# Dovecote
- Letters from Mary (Mary Epsen; "Marion Marigold" is her author pen name)
  - 9/1/74
    - Juni made it home safely
  - 9/28/74
    - this should be the fifth letter? Mising some dramatic lost letters
  - To herbet
    - Thanks for wedding doves, her and daniel have a new place in reddington
- We are returning east and should arrive tuesday, if coast is not clear lower the ?fd? to warn us

# Drafting Studio
- calendar with wall and apple orchard (Sat 7th Novemeber - Day one)    

# Drawing Room
- All the photos have something to do with step length?
- Candle stick on left - Safe

# Foundation
- Back of foundation has a lever you can spin to move lift
- Goes down to basement - Need basement key

# Foyer
- People
  - Lord William Epsen
  - Baron Tomas Sinclair
  - Major Nicolas Key IV
  - Je Ari Yenna
  - Baron Edwin Sinclair
  - Count Isaac Gates

# Freezer
- Not for staff use freezer
  - Has keys and gems in
- Need to find a way to disable power to freezer

# Furnace
- Powering with Boiler gives you a key (seems kinda lame)

# Garage
- Cristoff is disapearing
- Drop off address (14227 Sandy Heights)
- Suspicous car delivery to 14227 Sandy Heights
- goods in trunk
- Denny already has payment?

# Gemstone Cavern
- Two other ways in
- Stuck cart
- Second blue flame

# Great Hall
- has a switch for the antechamber
- Layout is in drafting studio

# Her Ladyships Chamber
- Suspect Clara Epsen's room
- Has a locked diary
- Drawing with spiral drawing saying "Does it never end"

# Hovel
- Had blue flame switch
- Note to Mr Ravane of termination - Last day 20th March

# Laundry
- Powered room
  - Steps <-> Diamonds
  - Allowance <-> Stars
  - die <-> keys

# Laboratory
- Something about not appearing for the first 3 days
- Switch board thing requires power from the boiler room
  - Unlocks Blackbridge grotto
- Periodic Table
  - PUSH THREE UP AFTER NINE
- Emails
  - H.S.Sinclair - 31/08/1987 - LOCK IT ALL DOWN
  - H.S.Sinclair - 27/08/1987 - Lock down computer in blackbridge grotto due to Redguard investigation
 - Chef - 05/04/1987 - To heat freezer draft furnance nearby

# Library
- Books read
  - A New Clue
    - Cuckoo clocks?
    - diction of each word was a sound?
  - Sightseer' guide to reddington
    - Names
      - Anne Babbage
      - Lord Evanson
      - M lowry
      - Simon P Jones
  - Swim Bird
    - Names
      - Hebert of Sinclair?
      - Mrs peterson
      - SIMON
      - Mrs Peterson
      - Anne Babage
      - Det Bon Margle
      - Simon P Jones
  - The Fixed Stars
  - EPCW Pump
  - The Curse of The Black Bridge
    - Jesper - Boy with dead parents and brother
      - Berry Picker??
      - Curse that he only has 12 more steps?
    - Miller & Ravia - Protecting a bridge
      - BlackGuard? Different to RedGuard?
  - Drafting Strategy Vol 3
    - Names
      - Kimberly Thompson
  - Drafting Strategy Vol 5 (20 dead end sweep stakes)
  - History or Orinda (Censored) - Suspect there is a uncensored version somewhere
    - Oris
    - Ajera
    - Fenn Aries
    - Arch Aries
    - Eraja
    - King Desilets III
    - War of fifth era
  - Realm & Rune
    - Same as book in the mines with pages ripped out
    - Not sure what this information is useful for yet

# Maid's Chambers
- Angel statue holding broom called MILA

# Mail room
- Denny Revane No longer staff
  - Did they ever prove he was the one that stole the Barons mail

# Master Bedroom
- Has signed copy of Red Prince with note I need magnifying glass for
  - PS they made me change the ending of course as you predicted
- Unfinished letter to Simon

# Music Room
- first words on the sheets are true message 9 10 (bold words)
- find among the white trees under two stones 9 10 (first word on page)
  - Found an observatory
- Denoted in verse?? cleff symbol??

# Office
- Photo of Simon H Sinclair and Clara Epsen
- Check if these books are avaialble
  - Flight of the Queen - 1
  - March of the count - 4
  - Fall of the lord - 0
  - Passing of the barons - 3
  - Herrings(hearings) of the Major - 0
- Greenhouse antechamber broken (only west and east work)
- Email
  - Hartley (Valet)
  - Fuse (off, blue, green, white, red, purple)
    - White only available after setting a 6 purple

# Outside
- Random door outside appeared in the rock? but it's not open
- Odd yellow pipe on lookout thing
- Can make a wish in well

# Pump Room
- Requireds Power to completely drain reservoir
- Draining Pool just reveals coins
- Draining Fountain gives 2nd entrance to Basement

# Trading Post
- Contains dynamite
- has some purple and yellow square puzzle I fucked up (edit: unlocked it and got an allowance)

# Room 46
- Lots of busts of different people
- Map of the world with 8 pins - Need to know the order of something? - Probably from the book
- Marigold statue missing


# Root Cellar
- Family Tree
- Note saying "no _sign_ has been found yet" of Marion Marigold since being since getting into a carriage last friday

# Rumpus room
- 1st
- black cat (red flag)
- Door rogue moon - has no key
- A lost room beyond the dark waters
- Should visit alazar again
- 2nd
- darkened cloud of ruin - disrepaired mansiona
- 8 red envelopes on 8 pedestals
- 8 letters by 8 different people
- Wel'll only find 7
- 3rd
- missing item in entrance hall - A bird statue??
- young boy watched from afar

# Schoolhouse
- Has tap for the wierd yellow pipes outside, lights a blue flame on the lookout (STAYS ON)
- desk says herbie
- "within a timely manor"
- give mrs peterson her gift?
- closing address
  - Mrs Hannah Peterson (future principle)
  - Baron sinclair (architect)

# Secret Garden
- Point both arrows at moon to unlock left antechamber door
- Use power hammer to break wall
  - Can point at East or West antechamber door
  - South shows a Sigil
    - Core - Orinda
    - Border - Black
    - Rays - 3
    - Moats - Metropolitan x5
    - Weather? - Fog

# Security
- Emails
  - Randolph Moore - 10/02/1987 - Stop the cameras recording
  - Kirk Darren - 24/01/1987 - South Lever in weight room - concealed

# Servants Quarters
- Green Note - There are Twelve Major Keys
- Hartley let go after 40 years
  - Given Lake House in Trinsdale
- Note in Bin
  - To Mr Ruota - RuotF - Deactivated
  - From Randolph Moore - IT person? also sorted out some experiments in the Laboratory
  - Temporary Admin Key - First 4 letters of last name followed by first initial, 1st and fith capitalized

# Shelter
- Has a time-lock safe
- What is the REDGUARD
- 3 People stayed down there during REDGUARD
- Need to figure out what day it is for timelock

# Shrine
- 10 coins - Blessing of the General - Something red rooms
- 5 Coins - Blessing of the Gardener - Add 8 courtyards

# Solarium
- Note on how to open Cloister

# Study
- Has a safe
- Black Queen on D8
- Unused pairings
  - Glove, sLing, crowD, gRown, sNack

# Tomb
- Herbert of Sinclair DEAD
- Orinda Aries stamp?
- Wished to be buried under the great oak overlooking plains
- Tree types (oak, sycamore, spruce)
- Is the tomb a dead END?
- Flames
  - 0,0 being bottom left
  - (0,0), (1,2), (2,1)
- Does the chapel statues match the tomb
  - YES, click them in order
- If you light the torches opens secret chamber with goodies

# Underground

## The Precipice
- Has a couple foremans log
- Has a book explaning the symbol I found in the secret garden

## The Underpass
- Red message 1 - May 8th
  - Dear Baron Sinclair
  - From Worth Laude
  - Blackmail letter?
- Theres a little castle underground - kinda cute

## Inner Sanctum
- Unlocked
  - Hour glass region


# Utility Closet
- Set these vac lights
  - Fuse (off, blue, green, white, red, purple)
- Opens outside to show gemstone cavern

# Vault
- Box 149
  - Diplomatic Passport for Samantha Hayes - Is just Mary Epsen??

# Workshop
- Letters - LCEMGMGILE (I'm pretty sure these are just the painting letters)
- Letter matched the images letter
- Recipes
  - Magnifying Glass + Metal Detector = Burning Glass
  - Shovel + Metal Detector = Detector Shovel
  - Shovel + Compass = Dowsing Rod
  - Sledge Hammer + Battery Pack + Lever = Power Hammer
  - Shovel + Battery Pack + Lever = Jack Hammer
  - Lock Pick + Metal Detector = Pick Sound Amplifier
  - Battery Pack + Compass = Electromagnet

# Gallery
- 4 artworks - I assume I have to figure out their names?
- THICK

# General (house-wide)
- Memos
  - Blue Memos - True
  - Red Memos - False
  - Red memos seen
    - Antechamber is unlocked and Access levers are no longer required
    - Keycard doors more common north
    - More locked doors north
- Terminals
  - Password: SWANSONG
  - Terminal rooms: Security, Office, Laboratory, Shelter
- Rooms have the same images if drafted in the same place
- June 15th, 1983
  - Lady Clara Espen Death
  - Out of respect don't visit west wing

# Keys
- There are 12 major keys (is this just a music joke?)
- Secret Garden Key
  - Use to unlock the secret garden
- Silver Key
  - Not sure 
- Prism Key
  - Unlocks room of same colour
- Safe Deposit Box - Key 149
  - No idea
- Safe Deposit Box - Key 233
  - No idea
- Safe deposit Box - Key 304
  - No idea

